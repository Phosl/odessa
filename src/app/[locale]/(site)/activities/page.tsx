...
export default async function ActivitiesPage({params}: {params: LocaleParams}) {
  ...
  return (
    <PageMain>
      ...
      <section className="section"><div className="container"><div className="grid-3">{page.items.map((activity, index) => <ActivityCard activity={activity} imageText={t('common.imagePlaceholder', {number: String(index + 1).padStart(2, '0')})} key={activity.id} />)}</div></div></section>
